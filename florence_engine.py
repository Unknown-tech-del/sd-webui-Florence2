    ...
    def run_task(self, image, task, prompt_text="", max_tokens=1024, temperature=0.7, top_p=0.9, do_sample=False):
        ...
        if is_qwen:
            qwen_prompts = {
                ...
                "Prompt Refine & Expand": f"Refine and enhance the following user prompt for creative text-to-image generation: '{prompt_text}'. Keep the meaning and keywords, make it more expressive and visually rich. Output ONLY the improved prompt text (no preface, no bullets, no JSON, no <think>, no commentary).",
